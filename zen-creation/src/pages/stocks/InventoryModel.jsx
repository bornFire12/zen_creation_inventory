    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 bg-[#FBFBF5] dark:bg-[#22231F] rounded-xl w-[700px] p-6 shadow-xl">
        ...
        <p className="text-lg text-center opacity-60 mt-1">
          Fill this Inventory Form and Your item will appear on Item View Section.
        </p>
        <div className="flex flex-col gap-0 rounded-lg border border-[#BDBDB8] mt-5.5 p-5.5 dark:bg-[#43433F] dark:border-none">
            ...
            <Input inputType={"text"} label="Name" value={item.name} onChange={(value) => setItem({...item, name: value})}/>
            <Input inputType={"number"} label="Size" value={item.size} onChange={(value) => setItem({...item, size: value})} />
            <Input inputType={"number"} label="Quantity" value={item.quantity} onChange={(value) => setItem({...item, quantity: Number(value)})} />
            <Input inputType={"text"} label="Color" value={item.color} onChange={(value) => setItem({...item, color: value})} />
            <Input inputType={"number"} label="Purchase Price" value={item.purchasePrice} onChange={(value) => setItem({...item, purchasePrice: Number(value)})} />
            <Input inputType={"number"} label="Selling Price" value={item.sellingPrice} onChange={(value) => setItem({...item, sellingPrice: Number(value)})} />
            ...
            <button className="cursor-pointer flex-1 bg-green-700 text-white py-2 rounded-lg text-lg" onClick={handleAdd}>
                Add Item
            </button>
            <button
                onClick={onClose}
...
const Field = ({ label, value }) => (
  <div>
    ...
    <div className="px-3 py-2 rounded mt-1 bg-[#EFEFEA] outline-none dark:text-[#22231F]">{value}</div>
  </div>
);
